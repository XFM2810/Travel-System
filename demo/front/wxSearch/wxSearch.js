...
//更新热门搜索和搜索记忆提示
function updateHotMindKeys(that,hotSearch){
  var temData = that.data.wxSearchData;
  var samekey = false,samemind = false;
  for (var i in temData.keys){
    if (temData.keys[i] == hotSearch){
        samekey = true;
        break;
    }
  }
  if (samekey == false){
    temData.keys.pop(); //删除热门搜索的最后一个元素
    temData.keys.unshift(hotSearch); //将新的热门搜索添加到数组开头
  }
  for (var i in __mindKeys) {
    if (__mindKeys[i] == hotSearch){
      samemind = true;
      break;
    }
  }
  if (samemind == false) {
    __mindKeys.pop(); //删除记录搜索的最后一个元素
    __mindKeys.unshift(hotSearch); //将新记录搜索添加到数组开头
  }
  that.setData({
    wxSearchData: temData
  });
...
module.exports = {
    ...
    updateHotMindKeys: updateHotMindKeys,
